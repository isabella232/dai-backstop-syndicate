...
contract("DaiBackstopSyndicate", (accounts: string[]) => {
  ...
  before(async () => {
    ...
    syndicateAbstract = await AbstractContract.fromArtifactName(
  ...
  beforeEach(async () => {
    ...
    // Deploy Authority
    authOwnerContract = (await authAbstract.deploy(
      ownerWallet
    )) as MkrAuthority;
    authContract = (await authOwnerContract.connect(
      userSigner
    )) as MkrAuthority;
    ...
    mkrOwnerContract = (await mkrAbstract.deploy(ownerWallet, [
      sym
    ])) as DSToken;
    ...
    // Deploy Flopper
    flopOwnerContract = (await flopAbstract.deploy(ownerWallet, [
      vatAddress,
      mkrAddress
    ])) as Flopper;
    ...
    daiJoinContract = (await daiJoinOwnerContract.connect(
      userSigner
    )) as DaiJoin;
    ...
    syndicateContract = (await syndicateOwnerContract.connect(
      userSigner
    )) as DaiBackstopSyndicate;
  ...
  context("When auction have NOT started", () => {
    describe("enlist() function", () => {
      ...
      it("should REVERT if user does not have enough dai", async () => {
        ...
        await expect(tx).to.be.rejectedWith(
          RevertError("Dai/insufficient-balance")
        );
      });
      ...
      context("When user enlisted", () => {
        ...
        it("should mint syndicate shares", async () => {
          let balance = await syndicateContract.functions.balanceOf(
            userAddress
          );
        ...
        it("should update syndicate VAT syndicate DAI balance", async () => {
          ...
          expect(vat_balance).to.be.eql(
            enlist_amount.mul(new BigNumber(10).pow(27))
          );
        });
    ...
    describe("defect() function", () => {
      it("should FAIL if user did not enlist", async () => {
        ...
        await expect(tx).to.be.rejectedWith(
          RevertError("SafeMath: subtraction overflow")
        );
      });
